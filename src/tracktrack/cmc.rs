// Módulo para la Compensación de Movimiento de Cámara (CMC) -> La saca de archivo pre-calculado por el tracker GMC.
...
pub struct Cmc {
    reader: Option<BufReader<File>>, // Ahora es opcional
}
...
/// Equivalente a la función apply_cmc de Python.
pub fn apply_cmc(tracks: &mut [Track], warp_matrix: &[[f32; 3]; 2]) {
    for track in tracks.iter_mut() {
        ...
        if let (Some(mean), Some(cov)) = (&mut track.mean, &mut track.covariance) {
            // 1. Construimos rot_8x8: El equivalente exacto a np.kron(np.eye(4), rot)
            ...
            // 3. Warp a la Media: rot_8x8 @ mean + traslación en X e Y
            *mean = rot_8x8 * (*mean);
            mean[0] += trans_x;
            mean[1] += trans_y;
